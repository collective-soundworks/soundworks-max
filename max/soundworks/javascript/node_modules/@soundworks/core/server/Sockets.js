...
class Sockets {
  ...
  start(httpServer, config, onConnectionCallback) {
    ...
    this.wss.on('connection', (ws, req) => {
      const queryString = _querystring.default.decode(req.url.split('?')[1]);
      ...
      if (!this._initializationCache.has(key)) {
        this._initializationCache.set(key, {
          ws,
          binary
        });
      } else {
        ...
        this._initializationCache.delete(key); // should be in order, but just to be sure


        const stringWs = cached.binary ? ws : cached.ws;
        const binaryWs = cached.binary ? cached.ws : ws;
        const socket = new _Socket.default(stringWs, binaryWs, this._rooms, this, config);
        socket.addToRoom('*');
  ...
  _broadcast(binary, roomIds, excludeSocket, channel, ...args) {
    ...
    targets.forEach(socket => {
      if (socket.ws.readyState === _ws.default.OPEN) {
