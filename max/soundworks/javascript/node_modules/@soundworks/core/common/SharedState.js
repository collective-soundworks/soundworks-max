...
${JSON.stringify(initValues, null, 2)}`);
    ...
    this._onDeleteCallbacks = new Set(); // add listener for state updates
    ...
    client.transport.addListener(`${_sharedStateUtils.DELETE_NOTIFICATION}-${id}-${this.remoteId}`, () => {
    ...
    if (this._isOwner) {
      ...
      // ---------------------------------------------
      client.transport.addListener(`${_sharedStateUtils.DELETE_RESPONSE}-${id}-${this.remoteId}`, reqId => {
        this._manager._statesById.delete(this.id);
      ...
      client.transport.addListener(`${_sharedStateUtils.DELETE_ERROR}-${id}`, (reqId, msg) => {
      ...
      // DETACH
      // ---------------------------------------------
      client.transport.addListener(`${_sharedStateUtils.DETACH_RESPONSE}-${id}-${this.remoteId}`, reqId => {
        this._manager._statesById.delete(this.id);
      ...
      client.transport.addListener(`${_sharedStateUtils.DETACH_ERROR}-${id}`, (reqId, msg) => {
        (0, _sharedStateUtils.rejectRequest)(reqId, msg);
      });
    }
  }
  ...
  _clearTransport() {
    // remove listeners
    this._client.transport.removeAllListeners(`${_sharedStateUtils.UPDATE_RESPONSE}-${this.id}-${this.remoteId}`);

    this._client.transport.removeAllListeners(`${_sharedStateUtils.UPDATE_NOTIFICATION}-${this.id}-${this.remoteId}`);

    this._client.transport.removeAllListeners(`${_sharedStateUtils.DELETE_RESPONSE}-${this.id}-${this.remoteId}`);

    this._client.transport.removeAllListeners(`${_sharedStateUtils.DELETE_NOTIFICATION}-${this.id}-${this.remoteId}`);

    this._client.transport.removeAllListeners(`${_sharedStateUtils.DETACH_RESPONSE}-${this.id}-${this.remoteId}`);
  }
  /** @private */
  ...
  async detach() {
    ...
    if (this._isOwner) {
      return new Promise((resolve, reject) => {
        ...
        this._client.transport.emit(`${_sharedStateUtils.DELETE_REQUEST}-${this.id}-${this.remoteId}`, reqId);
      });
    } else {
      return new Promise((resolve, reject) => {
        const reqId = (0, _sharedStateUtils.storeRequestPromise)(resolve, reject);

        this._client.transport.emit(`${_sharedStateUtils.DETACH_REQUEST}-${this.id}-${this.remoteId}`, reqId);
      });
    }
  }
  /**
