...
const encoder = new _windowOrGlobal.default.TextEncoder('utf-8');
...
/* arraybuffer */
  ...
  const startOffset = infos[2]; // need to slice as the library recreates a UInt8Array from the whole buffer
  ...
  const type = types[typeIndex]; // slice (copy) the underlying ArrayBuffer to create a clean TypedArray

  const data = new _windowOrGlobal.default[type](buffer.slice(startOffset));
  return [channel, data];
}
/** @private */
